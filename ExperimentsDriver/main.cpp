...
void WorkItem( _In_ PDEVICE_OBJECT DeviceObject,
               _In_ PVOID Context )
{
...
{
    auto status =
            PsSetCreateProcessNotifyRoutineEx( ProcessNotificationCallback,
                                               FALSE );
...
VOID TestExecutiveResourceConcurrentRead( )
{
    ...
    for ( ;; )
    {
        DbgPrint( "[%lu] Entering critical region",
                  threadid );
        KeEnterCriticalRegion ( );
        ExAcquireResourceSharedLite( &Res,
                                     TRUE );
        DbgPrint( "[%lu] Shared variable value: %lu",
                  threadid,
                  SharedVariable );
...
void GetSMBiosInformation( )
{
    ...
    /* Query struct size */
    status = AuxKlibGetSystemFirmwareTable( 'RSMB',
                                            0,
                                            nullptr,
                                            bufferSz,
                                            &neededBufferSz );
    ...
    {
        buffer = static_cast< SMBiosHeader* >( ExAllocatePoolWithTag( PagedPool,
                                                                      neededBufferSz,
                                                                      'SOIB' ) );
        if ( !buffer )
        {
        ...
        if ( NT_SUCCESS( status ) )
        {
            DbgPrint( "Got SMBios raw table\nMajor version: %d\nMinor version: %d\nLength: %lu\n",
                      buffer->SMBIOSMajorVersion,
                      buffer->SMBIOSMinorVersion,
                      buffer->Length );
        }
    }
...
void prcb_thread( void *context )
{
    static LARGE_INTEGER delay = { .QuadPart = -( 5 * 100 * 1000 ) };

    while ( true )
    {
        const auto prcb = __readgsqword( 0x20 );
        const auto current_cpu = KeGetCurrentProcessorIndex ( );

        DbgPrint( "thread id: %d cpu: %d prcb: %p\n",
...
NTSTATUS DriverIoctl( PDEVICE_OBJECT DeviceObject,
                      PIRP Irp )
{
